...
class PollingCursorAndKeyboardTracker {
    ...
    // Create a dedicated queue for CSV file writing (off the main thread)
    private let csvQueue = DispatchQueue(label: "com.yourapp.csvQueue", qos: .utility)
...
//        let mainDisplay = NSScreen.main
        ...
        // Instead of scheduling the CSV batch timer on the main thread,
        // we still schedule the timer on main (since Timer works with the run loop)
        // but call the writing on csvQueue.
        ...
        cursorPollingTask = Task(priority: .userInitiated) { [weak self] in
    ...
    func stopTracking() {
        ...
        // Write remaining batch on csvQueue
        csvQueue.async { [weak self] in
            self?.writeBatchToCSV()
            self?.updateEndTimeAndDuration()
            self?.closeCSVFile()
        }
...
extension PollingCursorAndKeyboardTracker: @unchecked Sendable {}
